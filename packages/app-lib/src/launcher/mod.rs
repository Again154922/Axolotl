...
pub(crate) struct GameVersionMetadata { pub world_version: Option<u32> }

pub(crate) async fn read_game_version_metadata_from_jar(_path: &Path) -> crate::Result<Option<GameVersionMetadata>> { Ok(None) }

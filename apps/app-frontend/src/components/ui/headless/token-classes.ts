...
export const headlessTokenClasses = {
	...
	dialogContent:
		'fixed left-1/2 top-1/2 z-50 w-[min(28rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 rounded-[var(--radius-lg)] border border-surface-5 bg-surface-2 p-6 text-contrast shadow-xl focus:outline-none',
	...
	checkboxRoot:
		'flex size-5 shrink-0 items-center justify-center rounded-[var(--radius-xs)] border border-surface-5 bg-surface-4 text-white transition-colors data-[state=checked]:border-brand data-[state=checked]:bg-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand',
	buttonStandard:
	...
	buttonBrand:
		'inline-flex h-9 items-center justify-center gap-2 rounded-[var(--radius-md)] border-0 bg-brand px-3 text-sm font-semibold text-white transition-[opacity,filter,transform] hover:brightness-[var(--hover-brightness)] active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand',
} as const
